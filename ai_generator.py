...
{slip_block}{deeplink_hint}{weekend_hint}{activation_warning}{products_hint}
ДАННЫЕ АКЦИИ:
- НОМЕР: {num}
- Название промо: {name}
- Сегмент: {segment}
...
14. «Кнопка» — текст кнопки + deeplink. Формат: «ТЕКСТ КНОПКИ deeplink».
    🧾 ЕСЛИ КАНАЛ = СЛИП-ЧЕК: поле «Кнопка» ВСЕГДА ПУСТОЕ (""). Слип бумажный, кнопки в нём нет. НЕ СТАВЬ «В КАТАЛОГ» и никакой deeplink.
