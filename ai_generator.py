            ...
            if _products:
                ...
                for _p in _products:
                    ...
                    if _disc:
                        _line += f" ({_disc})"
                    _items.append(_line)
                products_hint = "\n📦 АКТУАЛЬНЫЕ ТОВАРЫ ИЗ КАТАЛОГА ДИКСИ ДЛЯ ЭТОЙ АКЦИИ:\n  " + "\n  ".join(_items) + "\n  Используй эти РЕАЛЬНЫЕ товары в описании, тексте купона и push.\n  Не выдумывай товары — бери из этого списка!\n"
        except Exception:
            pass
...
{deeplink_hint}{weekend_hint}{activation_warning}{products_hint}
ДАННЫЕ АКЦИИ:
- НОМЕР: {num}
- Название промо: {name}
- Сегмент: {segment}
...
4. 🚨 «Механика» — ОБЯЗАТЕЛЬНО проверь название акции:
   - Если в названии есть слово «Активируй», «АКТИВИРУЙ», «активируй» (любым регистром) → «Механика» = «активация».
   - Если в названии «Акцептные» (предначисленные монеты) → «Механика» = «активация».
   - В остальных случаях → «Механика» = «автоматическая».
   ВАЖНО: пропустить активацию для акции с «Активируй» в названии — БРАК. Это меняет логику пуш и купона!
...
7. «Название информационного купона для МП» — КОРОТКОЕ название без дублирования с текстом купона!
   ⚠️ ВАЖНО: название купона + текст купона читаются СОВМЕСТНО как одно сообщение клиенту.
   Если в названии «Вернём 50%» — в тексте НЕ повторяй «вернём» / «с покупки».
   Если в названии «50% кешбэк» — в тексте начинай с «за покупку...» / «при покупке...».

   Шаблоны (БЕЗ повторов с текстом купона):
   - Для скидки: «Активируй скидку 10%»
   - Для кешбэка с активацией: «Активируй 20% кешбэк»
   - Для кешбэка без активации (вернём): «Вернём 50%» или «Вернём 100%» (без категории и без «с покупки»!)
   - Для бонуса/монет: «50% монетами» или «100₽ монетами» (без категории!)
   - Для подарка монет: «Активируй 50 монет»

   ❌ НЕЛЬЗЯ:
   - Название «Вернём 50% с покупки соусов» + текст «с покупки соусов...» (двойное «с покупки»)
