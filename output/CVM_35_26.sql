-- CVM неделя 35 (24.08–30.08.2026)
-- Выборки клиентов в I_PROMO_OFFER: конфеты и баланс баллов — широкая аудитория, полость рта — Активные/Новые.

-- 101355 · Активируй 20% на конфеты в коробках · Активные, Новые, Спящие, Отток · ср 26.08
-- 101355_Активируй 20 на конфеты в коробках
...
-- 101357_Скидка 20 на средства для ухода за полостью рта
...
;with p as (
select ID_PROMO = 101355, LIST_NAME = N'101355_Активируй 20 на конфеты в коробках', PROMO = N'Активируй 20% на конфеты в коробках', DATA_START = cast('2026-08-26' as date), MANZANA_ONLINE = N'да'
union all select 101356, N'101356_Баланс баллов', N'Баланс баллов', '2026-08-29', N'нет'
union all select 101357, N'101357_Скидка 20 на средства для ухода за полостью рта', N'Скидка 20% на средства для ухода за полостью рта', '2026-08-30', N'да'
)
...
from p
		...
		where ID_PROMO in (101355, 101356, 101357) and CONTROL_GROUP = 0
		group by ID_PROMO
		) o
...
select CRM_GUID
from I_PROMO_OFFER (nolock)
where ID_PROMO = 101355 and CONTROL_GROUP = 0
GO
...
select CRM_GUID
from I_PROMO_OFFER (nolock)
where ID_PROMO = 101356 and CONTROL_GROUP = 0
GO
...
select CRM_GUID
from I_PROMO_OFFER (nolock)
where ID_PROMO = 101357 and CONTROL_GROUP = 0
GO
